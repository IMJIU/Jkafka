...
private val stateChangeLogger = KafkaController.stateChangeLogger;
       ...
       public void handleStateChange(PartitionAndReplica partitionAndReplica, ReplicaState targetState,
        ...
        val Boolean leaderAndIsrIsEmpty =
        controllerContext.partitionLeadershipInfo.get(topicAndPartition) match {
       ...
       public Boolean  void areAllReplicasForTopicDeleted(String topic) {
       ...
       public Boolean  void isAtLeastOneReplicaInDeletionStartedState(String topic) {
        val replicasForTopic = controller.controllerContext.replicasForTopic(topic);
        val replicaStatesForTopic = replicasForTopic.map(r -> (r, replicaState(r))).toMap;
        replicaStatesForTopic.foldLeft(false)((deletionState, r) -> deletionState || r._2 == ReplicaDeletionStarted);
        }

       public void replicasInState(String topic, ReplicaState state): Set<PartitionAndReplica> = {
        replicaState.filter(r -> r._1.topic.equals(topic) && r._2 == state).keySet;
        }

       public Boolean  void isObjectReplicaInState(String topic, ReplicaState state) {
        replicaState.exists(r -> r._1.topic.equals(topic) && r._2 == state);
        }

       public void replicasInDeletionStates(String topic): Set<PartitionAndReplica> = {
...
privatepublic void assertValidPreviousStates(PartitionAndReplica partitionAndReplica, Seq fromStates<ReplicaState>,
        ...
        String.format(". Instead it is in %s state",replicaState(partitionAndReplica)))
        }

privatepublic void registerBrokerChangeListener() = {
        zkClient.subscribeChildChanges(ZkUtils.BrokerIdsPath, brokerChangeListener);
        }

privatepublic void deregisterBrokerChangeListener() = {
 ...
 * in zookeeper
 */
privatepublic void initializeReplicaState() {
