...
// kafka server;
       ...
       public Integer  parseControllerId(String controllerInfoString) {
              try {
                     Json.parseFull(controllerInfoString) match {
                            case Some(m) =>
                                   val controllerInfo = m.asInstanceOf<Map<String, Object>>
                                   return controllerInfo.get("brokerid").get.asInstanceOf<Integer>
       ...
       public Set<Broker> replicatedPartitionsBrokerLeads() {
               inLock(controllerContext.controllerLock) {
...
private void watchIsrChangesForReassignedPartition(String topic, Integer partition, ReassignedPartitionsContext reassignedPartitionContext) {
        val reassignedReplicas = reassignedPartitionContext.newReplicas;
...
privatepublic void updateLeaderEpochAndSendRequest(TopicAndPartition topicAndPartition, Seq replicasToReceiveRequest<Int], Seq newAssignedReplicas[Int>) {
        brokerRequestBatch.newBatch();
...
class PreferredReplicaElectionListener(KafkaController controller) extends IZkDataListener with Logging {
       ...
       public void handleDataDeleted(String dataPath) {
        ...
        case class ReassignedPartitionsContext(var Seq newReplicas<Integer> = Seq.empty,
        var ReassignedPartitionsIsrChangeListener isrChangeListener = null);


        }

        case class LeaderIsrAndControllerEpoch(val LeaderAndIsr leaderAndIsr, Int controllerEpoch) {
