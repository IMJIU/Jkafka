...
public class ZkUtils  extends Logging {
       ...
       public Optional<Integer> getLeaderForPartition(ZkClient zkClient, String topic, Integer partition){
        ...
        case None => None;
             ...
             */
           public Integer  void getEpochForPartition(ZkClient zkClient, String topic, Integer partition) {
                val leaderAndIsrOpt = readDataMaybeNull(zkClient, getTopicPartitionLeaderAndIsrPath(topic, partition))._1;
                leaderAndIsrOpt match {
        case Some(leaderAndIsr) =>
            Json.parseFull(leaderAndIsr) match {
           ...
           public void registerBrokerInZk(ZkClient zkClient, Integer id, String host, Integer port, Integer timeout, Integer jmxPort) {
            ...
            try {
                createEphemeralPathExpectConflictHandleZKBug(zkClient, brokerIdPath, brokerInfo, expectedBroker,
                        (String brokerString, Any broker) => Broker.createBroker(broker.asInstanceOf<Broker].id, brokerString).equals(broker.asInstanceOf[Broker>),
                        timeout);
       ...
       public String  void getConsumerPartitionOwnerPath(String group, String topic, Integer partition) {
                val topicDirs = new ZKGroupTopicDirs(group, topic);
                topicDirs.consumerOwnerDir + "/" + partition;
        }


       public String  void leaderAndIsrZkData(LeaderAndIsr leaderAndIsr, Integer controllerEpoch) {
         ...
         */
       public String  void replicaAssignmentZkData(Map map<String, Seq<Integer>>) {
       ...
       public void makeSurePersistentPathExists(ZkClient client, String path) {
         ...
         */
    privatepublic Unit  void createParentPath(ZkClient client, String path) {
     ...
     */
    privatepublic Unit  void createEphemeralPath(ZkClient client, String path, String data) {
        try {
     ...
     */
   public Unit  void createEphemeralPathExpectConflict(ZkClient client, String path, String data) {
        try {
     ...
     */
   public Unit  void createEphemeralPathExpectConflictHandleZKBug(ZkClient zkClient, String path, String data, Any expectedCallerData, checker: (String, Any) => Boolean, Integer backoffTime) {
        while (true) {
            try {
   ...
   public String  void createSequentialPersistentPath(ZkClient client, String path, String data = "") {
   ...
   public void conditionalUpdatePersistentPathIfExists(ZkClient client, String path, String data, Integer expectVersion): (Boolean, Int) = {
     ...
     * create parrent directory if necessary. Never throw NodeExistException.
     */
   public Unit  void updateEphemeralPath(ZkClient client, String path, String data) {
        try {
            client.writeData(path, data);
        } catch {
            case ZkNoNodeException e => {
   ...
   public Boolean  void deletePath(ZkClient client, String path) {
        try {
            client.delete(path);
        } catch {
            case ZkNoNodeException e =>
   ...
   public void readDataMaybeNull(ZkClient client, String path): (Option<String>, Stat) = {
        val Stat stat = new Stat();
        val dataAndStat = try {
   ...
   public void getChildren(ZkClient client, String path): Seq<String> = {
    import scala.collection.JavaConversions._;
        // triggers implicit conversion from java list to scala Seq;
        client.getChildren(path);
    }

   public void getChildrenParentMayNotExist(ZkClient client, String path): Seq<String> = {
    import scala.collection.JavaConversions._;
        // triggers implicit conversion from java list to scala Seq;
        try {
            client.getChildren(path);
        } catch {
            case ZkNoNodeException e => return Nil;
            case Throwable e2 => throw e2;
        }
    }
     ...
     */
   public Boolean  void pathExists(ZkClient client, String path) {
        client.exists(path);
    }

   public Cluster  void getCluster(ZkClient zkClient)  {
  ...
  : mutable.Map<TopicAndPartition, LeaderIsrAndControllerEpoch> = {
   ...
   public void getPartitionAssignmentForTopics(ZkClient zkClient, Seq topics<String>): mutable.Map<String, collection.Map<Integer,> Seq<Integer>>> = {
   ...
   public String  void getPartitionReassignmentZkData(Map partitionsToBeReassigned<TopicAndPartition, Seq<Integer>>) {
