...
public class KafkaConfig {
     ...
     */
    class KafkaConfig private (val ) extends ZKConfig(props) {

        public void  this(Properties originalProps) {
            this(new VerifiableProperties(originalProps))
            props.verify()
        }

    private public void  getLogRetentionTimeMillis(): Long = {
    ...
    private public void  getLogRollTimeMillis(): Long = {
    ...
    private public void  getLogRollTimeJitterMillis(): Long = {
        val millisInHour = 60L * 60L * 1000L;
  ...
  /* the broker id for this server */
    val Integer brokerId = props.getIntInRange("broker.id", (0, Int.MaxValue));
  ...
  /* the port to listen and accept connections on */
    ...
    val String advertisedHostName = props.getString("advertised.host.name", hostName);
    ...
    val Integer advertisedPort = props.getInt("advertised.port", port);

    /* the SO_SNDBUFF buffer of the socket sever sockets */
    val Integer socketSendBufferBytes = props.getInt("socket.send.buffer.bytes", 100*1024);

    /* the SO_RCVBUFF buffer of the socket sever sockets */
    val Integer socketReceiveBufferBytes = props.getInt("socket.receive.buffer.bytes", 100*1024);

    /* the maximum number of bytes in a socket request */
    val Integer socketRequestMaxBytes = props.getIntInRange("socket.request.max.bytes", 100*1024*1024, (1, Int.MaxValue));

    /* the maximum number of connections we allow from each ip address */
    val Integer maxConnectionsPerIp = props.getIntInRange("max.connections.per.ip", Int.MaxValue, (1, Int.MaxValue));
  ...
  /* the maximum size for a metadata entry associated with an offset commit */
    ...
    val Short offsetsTopicReplicationFactor = props.getShortInRange("offsets.topic.replication.factor",
    OffsetManagerConfig.DefaultOffsetsTopicReplicationFactor, (1, Short.MaxValue));

    /** The number of partitions for the offset commit topic (should not change after deployment). */
    val Integer offsetsTopicPartitions = props.getIntInRange("offsets.topic.num.partitions",
    OffsetManagerConfig.DefaultOffsetsTopicNumPartitions, (1, Integer.MAX_VALUE));

    /** The offsets topic segment bytes should be kept relatively small in order to facilitate faster log compaction and cache loads */
    val Integer offsetsTopicSegmentBytes = props.getIntInRange("offsets.topic.segment.bytes",
    ...
    val Integer offsetsRetentionMinutes = props.getIntInRange("offsets.retention.minutes", 24*60, (1, Integer.MAX_VALUE));

    /** Frequency at which to check for stale offsets. */
    val Long offsetsRetentionCheckIntervalMs = props.getLongInRange("offsets.retention.check.interval.ms",
